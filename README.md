# wallpaper-collection
A personal collection of wallpapers. Taken from many sites such as:
- Various Omarchy Themes (https://omarchythemes.com/ and https://omarchy.deepakness.com/themes)
- Nord wallpapers repos (https://github.com/linuxdotexe/nordic-wallpapers, https://github.com/dxnst/nord-backgrounds, https://github.com/TheSerphh/Nord-Wallpapers)
- GNOME wallpapers repo (https://zebreus.github.io/all-gnome-backgrounds/)
- KDE Wallpapers repo (https://github.com/KDE/plasma-workspace-wallpapers/tree/master, https://invent.kde.org/plasma/breeze/-/tree/master/wallpapers/Next/contents/images)
- Gruvbox Wallpapers (https://gruvbox-wallpapers.pages.dev/)
- LinuxNext's Wallpaper repo (https://gitlab.com/Pollux78/linuxnext-wallpapers/-/tree/main)
- Linux Mint Wallpaper repo (https://github.com/rt2yrru/linux_mint_wallpaper)
- Wallhaven
- MacOS wallpapers (https://512pixels.net/projects/default-mac-wallpapers-in-5k/)
- Aesthetic Wallpapers repo (https://github.com/D3Ext/aesthetic-wallpapers/tree/main)
- Distro Wallpapers (https://distrowall.pages.dev/)
- Ubuntu (https://ubuntu.com/blog/unmasking-the-resolute-raccoon)
- XeroLinux (https://github.com/xerolinux/kde-wallpapers-extra)

Be sure to check these websites out, you will find even more cool wallpapers there!
